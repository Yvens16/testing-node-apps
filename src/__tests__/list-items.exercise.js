...
test('listItem CRUD', async () => {
  ...
  const rData = await authAPI.get(listItemIdUrl, cData.listItem)
  expect(rData.listItem).toEqual(cData.listItem)
  expect(rData.listItem).toMatchObject({
    ownerId: testUser.id,
    bookId: book.id,
  })
  ...
  expect(pData.listItem.notes).toEqual(updates.notes)
  // DELETE
  // 🐨 make a DELETE request to the `listItemIdUrl`
  // 🐨 assert that this returns the right stuff (💰 {success: true})
  const dData = await authAPI.delete(listItemIdUrl, rData.listItem)
